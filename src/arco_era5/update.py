...
def generate_offset(url: str, timestamps_per_file: int = 24, init_date: str = '1900-01-01', level: str = "model"):
    ...
    end = start + timestamps_per_file * (calendar.monthrange(start_date.year, start_date.month)[1] if level == "single" else 1)
    return url, slice(start, end), variable_dict[chunk]


def update(file_slice: tuple[str, slice], target: str):
...
class GenerateOffset(beam.PTransform):
    ...
    def expand(self, pcoll: beam.PCollection) -> beam.PCollection:
        return pcoll | beam.Map(generate_offset, init_date=self.init_date, timestamps_per_file=self.timestamps_per_file, level=self.level)
...
class UpdateSlice(beam.PTransform):
    ...
    def expand(self, pcoll: beam.PCollection) -> beam.PCollection:
        return pcoll | beam.Map(update, target=self.target)
